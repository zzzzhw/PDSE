...
def normalized_weight_diff(model, proxy, parameter_prefix='encoder_model.'):
    """Return the scale-normalized proxy update used by the W-DOE code."""
...
def build_exposure_indices(y_family, y_binary, exposure_count, seed=0,
    ...
    for current_index in current_indices:
        ...
        if same_candidates.size == 0 or opposite_candidates.size == 0:
            raise ValueError('IDS exposure construction requires both benign and malicious history')
...
class HCLIDS:
    """First-order IDS adaptation for the hierarchical contrastive classifier."""

    def __init__(self, model, args, device):
        if not hasattr(model, 'encoder_model'):
            raise ValueError('HCL+IDS requires a model with an encoder_model module')
        self.device = device
        dropout = next(
        ...
        self.update_mode = args.ids_hcl_update_mode
        self.diff = None
