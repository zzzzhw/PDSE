...
def parse_args():
    ...
    p.add_argument('--test_start', help='First test_valuation month.')
    p.add_argument('--test_end', help='Last test_valuation month.')
    ...
    p.add_argument('--accumulate_data', action='store_true', help='Whether to accumulate test_valuation data from previous month, excluding the selected test_valuation samples')
    ...
    p.add_argument('--encoder_retrain', action='store_true',
    ...
    p.add_argument('--sampler', type=str, choices=['mperclass', 'proportional', 'half', \
                    'triplet', 'random'], help='The sampler to sample batches.')
    ...
    # 设置F1-D的D值
    p.add_argument('--F1D', type=int, default=6, help='The D value of F1-D')
