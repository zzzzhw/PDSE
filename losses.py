...
class TripletLoss(nn.Module):
    ...
    def forward(self, features, labels = None, margin = 10.0,
        ...
        Args:
            ...
            split: whether it is in test_valuation data
        Returns:
            A loss scalar.
        """
...
class HiDistanceLoss(nn.Module):
    ...
    def forward(self, features, binary_cat_labels, labels = None, margin = 10.0,
        ...
        Args:
            ...
            weight: sample weights to adjust the sample loss values
            split: whether it is in test_valuation data, so we ignore entries for these
        Returns:
            A loss scalar.
        """
        device = (torch.device('cuda')
        ...
        # adjust the masks based on test_valuation indices
...
class HiDistanceXentLoss(nn.Module):
    ...
    def forward(self, xent_lambda,
        ...
        Args:
            ...
            weight: sample weights to adjust the sample loss values
            split: whether it is in test_valuation data, so we ignore these entries
        Returns:
            A loss scalar.
        """
