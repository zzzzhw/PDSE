...
case ${DATA} in
    gen_androzoo_drebin)
        ...
        DEFAULT_PDSE_BALANCED_EXPOSURE=0
        DEFAULT_PDSE_ROBUST_WEIGHT=1.0
        ;;
    gen_apigraph_drebin)
    ...
    kronodroid|kronodroid_real_2008_2014)
        ...
        DEFAULT_HCL_GRAD_CLIP=10.0
        DEFAULT_PDSE_BALANCED_EXPOSURE=1
        DEFAULT_PDSE_ROBUST_WEIGHT=0.5
        ;;
    *)
...
PDSE_BALANCED_EXPOSURE=${PDSE_BALANCED_EXPOSURE:-${DEFAULT_PDSE_BALANCED_EXPOSURE}}
PDSE_ROBUST_WEIGHT=${PDSE_ROBUST_WEIGHT:-${DEFAULT_PDSE_ROBUST_WEIGHT}}

PDSE_EXPOSURE_ARGS=()
if [ "${PDSE_BALANCED_EXPOSURE}" = "1" ]; then
    PDSE_EXPOSURE_ARGS+=(--pdse-balanced-exposure)
fi

RESULT_DIR=${RESULT_DIR:-results/hcl_pdse_combined}
AL_OPT=${AL_OPT:-adam}
...
PDSE_LAMBDA=${PDSE_LAMBDA:-0.0025}
PDSE_GAMMA=${PDSE_GAMMA:-0}
