...
class UncertainPredScoreSelector(Selector):
    ...
    def select_samples(self, args, X_test, y_test_pred, total_count, \
                       ...
                       total_wrong=0, \
                       max_count=None):
        # offset indicates where to start in test_valuation samples.
        # we will sort prediction scores of all test_valuation samples
        offset = 0
        self.sample_indices = []
        if args.classifier not in ['svm', 'gbdt']:
            ...
            X_test_tensor = X_test_tensor.reshape(-1, 34, 34)
            X_test_tensor = X_test_tensor.unsqueeze(0)
            X_test_tensor = X_test_tensor.permute(1, 0, 2, 3)
